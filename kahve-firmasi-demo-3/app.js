/* Holly Coffee Roastery - Option 3: Luxury Dark Storefront Script */
...
function loadProducts() {
    const savedProducts = localStorage.getItem('holly_coffee_products_3');
    if (savedProducts) {
        try {
        ...
        products = [...defaultProducts];
        localStorage.setItem('holly_coffee_products_3', JSON.stringify(products));
    }
}
...
function loadOrders() {
    const savedOrders = localStorage.getItem('holly_coffee_orders_3');
    if (savedOrders) {
        try {
        ...
        localStorage.setItem('holly_coffee_orders_3', JSON.stringify(orders));
    }
}
...
function loadCustomers() {
    const savedCustomers = localStorage.getItem('holly_coffee_customers_3');
    if (savedCustomers) {
        try {
        ...
        localStorage.setItem('holly_coffee_customers_3', JSON.stringify(customers));
    }
}
...
function loadCartFromLocalStorage() {
    const savedCart = sessionStorage.getItem('holly_coffee_cart_3');
    if (savedCart) {
        try {
...
function saveCartToSessionStorage() {
    sessionStorage.setItem('holly_coffee_cart_3', JSON.stringify(cart));
}

function checkActiveUserSession() {
    const savedUser = sessionStorage.getItem('holly_coffee_user_3');
    if (savedUser) {
        try {
...
// --- Theme Controller (Blue / Pink Toggle) ---
function initializeTheme() {
    const savedTheme = localStorage.getItem('holly_coffee_theme_3');
    if (savedTheme) {
...
function toggleTheme() {
    ...
    document.body.className = `theme-${currentTheme}`;
    localStorage.setItem('holly_coffee_theme_3', currentTheme);
}
    ...
    productsList.forEach(prod => {
        ...
        card.innerHTML = `
            <div class="product-image-container">
                ...
                ${prod.roastLevel > 0 ? `<span class="product-badge">Çekirdek</span>` : `<span class="product-badge" style="border-color:#555; color:#aaa;">Ekipman</span>`}
            </div>
...
function handleCustomerLogin(email, password) {
    ...
    if (user) {
        currentUser = user;
        sessionStorage.setItem('holly_coffee_user_3', JSON.stringify(currentUser));
        updateUserMenuState(true);
...
function processCheckoutPayment() {
    ...
    cart = [];
    sessionStorage.removeItem('holly_coffee_cart_3');
    updateCartBadge();
    closeCartDrawer();
...
function setupEventListeners() {
    ...
    document.getElementById('user-logout-btn').addEventListener('click', () => {
        currentUser = null;
        sessionStorage.removeItem('holly_coffee_user_3');
