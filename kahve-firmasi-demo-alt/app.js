/* Holly Coffee Roastery - Alternative Design Storefront Script */
...
function loadProducts() {
    const savedProducts = localStorage.getItem('holly_coffee_products_alt');
    if (savedProducts) {
        try {
        ...
        products = [...defaultProducts];
        localStorage.setItem('holly_coffee_products_alt', JSON.stringify(products));
    }
}
...
function loadOrders() {
    const savedOrders = localStorage.getItem('holly_coffee_orders_alt');
    if (savedOrders) {
        try {
        ...
        localStorage.setItem('holly_coffee_orders_alt', JSON.stringify(orders));
    }
}
...
function loadCustomers() {
    const savedCustomers = localStorage.getItem('holly_coffee_customers_alt');
    if (savedCustomers) {
        try {
        ...
        localStorage.setItem('holly_coffee_customers_alt', JSON.stringify(customers));
    }
}
...
function loadCartFromLocalStorage() {
    const savedCart = sessionStorage.getItem('holly_coffee_cart_alt');
    if (savedCart) {
        try {
...
function saveCartToSessionStorage() {
    sessionStorage.setItem('holly_coffee_cart_alt', JSON.stringify(cart));
}

function checkActiveUserSession() {
    const savedUser = sessionStorage.getItem('holly_coffee_user_alt');
    if (savedUser) {
        try {
...
// --- Theme Controller (Light/Dark Switcher) ---
function initializeTheme() {
    const savedTheme = localStorage.getItem('holly_coffee_theme_alt');
    if (savedTheme) {
...
function toggleTheme() {
    ...
    document.body.className = `theme-${currentTheme}`;
    localStorage.setItem('holly_coffee_theme_alt', currentTheme);
    updateThemeIcon();
}
...
function renderProducts(productsList) {
    ...
    productsList.forEach(prod => {
        ...
        card.innerHTML = `
            <div class="product-image-container">
                ...
                ${prod.roastLevel > 0 ? `<span class="product-badge">Çekirdek</span>` : `<span class="product-badge" style="background:#555;">Ekipman</span>`}
            </div>
    ...
    // Add listeners to new Add to Cart buttons
...
function updateCartBadge() {
    ...
    // Enable checkout button if items present
...
function updateUserMenuState(isLoggedIn) {
    ...
    if (isLoggedIn && currentUser) {
        // Change icon to solid user, color gold
        icon.className = 'fa-solid fa-user';
        icon.style.color = 'var(--gold-color)';
...
function handleCustomerLogin(email, password) {
    ...
    if (user) {
        currentUser = user;
        sessionStorage.setItem('holly_coffee_user_alt', JSON.stringify(currentUser));
        updateUserMenuState(true);
...
function processCheckoutPayment() {
    ...
    // Clear Cart
    cart = [];
    sessionStorage.removeItem('holly_coffee_cart_alt');
    updateCartBadge();
    closeCartDrawer();
...
function showQuizRecommendation() {
    let matchId = 'prod-1'; // Default: Blend
    const brew = quizAnswers[1];
    const taste = quizAnswers[2];
    
    if (brew === 'filtre' && taste === 'meyve') {
        matchId = 'prod-2'; // Ethiopia Light Roast
    } else if (brew === 'filtre' && taste === 'dengeli') {
        matchId = 'prod-3'; // Colombia Medium Roast
    } else if (brew === 'espresso') {
        matchId = 'prod-1'; // Espresso Blend
    } else {
        matchId = 'prod-3'; // Default Colombia
    }
...
function renderAdminOrders() {
    ...
    if (orders.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center" style="opacity:0.5;">Herhangi bir sipariş kaydı bulunamadı.</td></tr>`;
        return;
    }
...
function renderAdminProducts() {
    ...
    products.forEach(p => {
        ...
        tr.innerHTML = `
            <td>
                <div class="admin-img-preview"><i class="fa-solid ${p.icon}"></i></div>
            </td>
        ...
        btn.addEventListener('click', () => {
            ...
            if (input) {
                ...
                if (product && !isNaN(newPrice) && newPrice > 0) {
                    ...
                    renderProducts(products); // Redraw customer catalog!
...
function setupEventListeners() {
    ...
    document.getElementById('user-logout-btn').addEventListener('click', () => {
        currentUser = null;
        sessionStorage.removeItem('holly_coffee_user_alt');
        updateUserMenuState(false);
    });

    // Open Orders Log History modal
    ...
    // Back to store from admin login screen
    ...
    // Admin login form submit validation
    ...
    // Sidebar tab switching listeners
    ...
    // Back to store from admin dashboard
